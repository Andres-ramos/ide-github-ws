...
def create_app() -> None:
    ...
    def render_temperatures():
        temperature = get_db().execute(
            'SELECT temperature, time FROM temperature'
        ).fetchall()
        temperatures_json = [{"temp": entry[0], "date": entry[1].strftime("%m/%d/%Y/%H")} for entry in temperature]
        table_data= temperatures_json
        
        temperature_list = [entry[0] for entry in temperature]
